...
public static class LerpEx {
	...
	public static float ScaleLerp ( float from, float to, float from2, float to2, float value ) {
		if ( from2 < to2 ) {
			if ( value < from2 ) {
				value = from2;
			} else if ( value > to2 ) {
	...
	public static float ScaleLerpUnclamped ( float from, float to, float from2, float to2, float value ) {
		return ( to - from )*( ( value - from2 ) / ( to2 - from2 ) ) + from;
	}
	...
	public static Quaternion QuaternionLerp ( Quaternion q1, Quaternion q2, float value ) {
		if ( value > 1.0f ) {
			return q2;
		} else if ( value < 0.0f ) {
			return q1;
		...
		return new Quaternion ( q1.x + ( q2.x - q1.x )*value,
		                        q1.y + ( q2.y - q1.y )*value,
		                        q1.z + ( q2.z - q1.z )*value,
		                        q1.w + ( q2.w - q1.w )*value );
	}
	
	public static Quaternion QuaternionLerpUnclamped ( Quaternion q1, Quaternion q2, float value ) {
		return new Quaternion ( q1.x + ( q2.x - q1.x )*value,
		                        q1.y + ( q2.y - q1.y )*value,
		                        q1.z + ( q2.z - q1.z )*value,
		                        q1.w + ( q2.w - q1.w )*value );
